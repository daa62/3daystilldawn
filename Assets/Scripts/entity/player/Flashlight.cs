...
// Two-light flashlight: a narrow "throw" beam for distance plus a dim wide "fill"
// for the near foreground. Only drives the toggle and sway — intensity/colour/
// shadows are authored on the lights themselves, so lumen values stick.
public class Flashlight : MonoBehaviour
{
    ...
    // handheld feel: the beam trails a turn slightly and bobs a little while walking
    void updateSway()
    {
        // exp easing so it's frame-rate independent
        float ease = 1f - Mathf.Exp(-swaySmooth * Time.deltaTime);

        // raw mouse delta is jittery, damp it before using it for look lag
        ...
        if (moving)
        {
            // cap the speed factor — raw sprint speed made the bob frantic
